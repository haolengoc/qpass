  ...
  return (
    <section className="space-y-8">
      ...
      <div className="grid gap-px overflow-hidden rounded-md border bg-border sm:grid-cols-2 lg:grid-cols-4">
        {metrics.map(({ label, value, icon: Icon }) => <div className="bg-background p-5" key={label}><Icon className="h-5 w-5 text-primary" aria-hidden="true" /><p className="mt-4 text-sm text-muted-foreground">{label}</p><p className="mt-1 text-3xl font-semibold">{value}</p></div>)}
      </div>
